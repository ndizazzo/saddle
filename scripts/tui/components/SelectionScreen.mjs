  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        h(Text, {}, " "),
        h(Text, {}, "   "),
        h(Text, { color: theme.color.fg.dim }, "CLI"),
      ),
      h(
        Box,
        { columnGap: 2 },
        h(Box, { width: 6, justifyContent: "flex-end" }, h(Text, { color: theme.color.fg.dim }, "EXIST")),
        h(Box, { width: 5, justifyContent: "flex-end" }, h(Text, { color: theme.color.fg.dim }, "ACT")),
      ),
    ),
...
export function ActionSelector({ toolGroups, toolIndex, flatActions, selectedActionKeys, actionIndex, actionScrollOffset, layout }) {
  ...
  const actionAreaHeight = Math.max(1, layout.mainHeight - layout.sourceHeight - profileIntroLines.length - 9);
