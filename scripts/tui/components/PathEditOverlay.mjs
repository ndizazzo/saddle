...
export function PathEditOverlay({ currentValue, onSubmit, onCancel, layout }) {
  ...
  return h(
    ...
    h(
      ...
      h(Text, { color: theme.color.fg.muted }, "Path to your ai-config repository:"),
      h(
        Box,
        { borderStyle: "single", borderColor: theme.color.border.subtle, paddingX: 1, marginTop: 1 },
        h(TextInput, { defaultValue: currentValue, placeholder: "~/path/to/ai-config", onSubmit: handleSubmit }),
      ),
      h(Box, { marginTop: 1 },
