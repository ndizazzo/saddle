...
require_file "scripts/symlink-home.sh"
require_executable "scripts/symlink-home.sh"
...
require_file "bin/ai-config.js"
require_executable "bin/ai-config.js"
require_file ".husky/pre-commit"
require_executable ".husky/pre-commit"
require_file "package.json"
require_json "package.json"

node <<'EOF' >/dev/null 2>&1 || fail "package.json is missing husky/lint scripts"
const fs = require("fs");
const pkg = JSON.parse(fs.readFileSync("package.json", "utf8"));
if (pkg.private !== true) process.exit(1);
if (pkg.scripts?.prepare !== "husky") process.exit(1);
if (pkg.scripts?.["lint:agents"] !== "bash ./scripts/agent-lint.sh") process.exit(1);
if (pkg.bin?.["ai-config"] !== "./bin/ai-config.js") process.exit(1);
...
if ! grep -q -- '--profile' scripts/symlink-home.sh; then
  fail "scripts/symlink-home.sh must remain a compatibility wrapper with --profile"
fi
